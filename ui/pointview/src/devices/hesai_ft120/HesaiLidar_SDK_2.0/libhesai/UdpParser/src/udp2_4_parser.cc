...
// The main purpose of this function is to open, read, and parse a lidar
// calibration file, and print appropriate messages based on the parsing
// results. This function can read both .bin and .csv files.
template <typename T_Point>
void Udp2_4Parser<T_Point>::LoadCorrectionFile(
...
// csv ----> correction
template <typename T_Point>
int Udp2_4Parser<T_Point>::LoadCorrectionString_csv(
    std::string lidar_correction_file) {
...
int Udp2_4Parser<T_Point>::LoadCorrectionString(char* data) {
  try {
    char* p = data;
    struct ETCorrections_v4_Header ETheader =
        *((struct ETCorrections_v4_Header*)p);
    if (0xee == ETheader.delimiter[0] && 0xff == ETheader.delimiter[1]) {
      switch (ETheader.min_version) {
        case 1: {
          memcpy((void*)&m_ET_corrections, p,
                 sizeof(struct ETCorrections_v4_Header));
          ...
          m_ET_corrections.elevations[0] =
              ((float)(m_ET_corrections.apha)) / division;
          m_ET_corrections.elevations[1] =
              ((float)(m_ET_corrections.beta)) / division;
          m_ET_corrections.elevations[2] =
              ((float)(m_ET_corrections.gamma)) / division;
          printf("apha:%f, beta:%f, gamma:%f\n", m_ET_corrections.elevations[0],
                 m_ET_corrections.elevations[1],
                 m_ET_corrections.elevations[2]);
          for (int i = 0; i < channel_num; i++) {
            m_ET_corrections.azimuths[i + 3] =
                ((float)(m_ET_corrections.raw_azimuths[i])) / division;
            m_ET_corrections.elevations[i + 3] =
                ((float)(m_ET_corrections.raw_elevations[i])) / division;
            printf("%d %f %f \n", i, m_ET_corrections.azimuths[i + 3],
                   m_ET_corrections.elevations[i + 3]);
          }
...
template <typename T_Point>
int Udp2_4Parser<T_Point>::DecodePacket(LidarDecodedPacket<T_Point>& output,
  ...
  for (int blockId = 0; blockId < pHeader->GetBlockNum(); blockId++) {
    for (int seqId = 0; seqId < pHeader->GetSeqNum(); seqId++) {
      ...
      for (int unitId = 0;
           unitId < (pHeader->GetLaserNum() / pHeader->GetSeqNum()); unitId++) {
      ...
      pSeq2 = reinterpret_cast<const HS_LIDAR_BODY_SEQ2_ET_V4*>(
          (const unsigned char*)pSeq2 + sizeof(HS_LIDAR_BODY_SEQ2_ET_V4) +
          sizeof(HS_LIDAR_BODY_LASTER_UNIT_ET_V4) *
              (pHeader->GetLaserNum() / pHeader->GetSeqNum()));
...
template <typename T_Point>
int Udp2_4Parser<T_Point>::ComputeXYZI(LidarDecodedFrame<T_Point>& frame,
                                       LidarDecodedPacket<T_Point>& packet) {
  ...
  for (int blockId = 0; blockId < packet.block_num; blockId++) {
    ...
    for (int i = 0; i < lasernum; i++) {
      int point_index = packet.packet_index * packet.points_num +
                        blockId * packet.laser_num + i;
      ...
      float distance =
          packet.distances[i + blockId * lasernum] * packet.distance_unit;
      ...
      float elv_v = raw_elevation * M_PI / 180 + theta * M_PI / 180 -
                    std::tan(raw_elevation * M_PI / 180) *
                        (1 - std::cos(an * M_PI / 180));
      float delt_azi_v = std::sin(an * M_PI / 180) * std::cos(an * M_PI / 180) *
                         theta_n * theta_n / 2 * 1.016 * M_PI / 180 * M_PI /
                         180;
      float eta = phi + delt_azi_v * 180 / M_PI + beta + raw_azimuth / 2;
      float delt_azi_h = std::sin(eta * M_PI / 180) *
                             std::tan(2 * gamma * M_PI / 180) *
                             std::tan(elv_v) +
                         std::sin(2 * eta * M_PI / 180) * gamma * gamma * M_PI /
                             180 * M_PI / 180;
      float elv_h = elv_v * 180 / M_PI + std::cos(eta * M_PI / 180) * 2 * gamma;
      float azi_h = 90 + raw_azimuth + delt_azi_h * 180 / M_PI +
                    delt_azi_v * 180 / M_PI + phi;
      ...
      setIntensity(frame.points[point_index],
                   packet.reflectivities[blockId * packet.laser_num + i]);
      setTimestamp(frame.points[point_index],
                   double(packet.sensor_timestamp) / kMicrosecondToSecond);
