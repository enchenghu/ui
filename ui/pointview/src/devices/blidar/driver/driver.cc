...
std::array<double, kRotationMaxUnits> Driver::sin_lookup_table_;
std::array<double, kRotationMaxUnits> Driver::cos_lookup_table_;
...
void Driver::setPointCloudCallback(PointCloudCallbackType cb) {
...
void Driver::resetDriverLoad() { index_f_single_thread_ = 0; }
bool Driver::ParseLidarPacket(const uint8_t* data, size_t len) {
  // parse udp packet
...
bool Driver::ParsePacket(const uint8_t* data, size_t len,
...
void Driver::CalcPointXYZIT(const LidarPacket* udp_pkt, unsigned int block_id,
...
float Driver::CosLookupDegree(float angle_in_deg) {
  if (angle_in_deg < 0) {
    angle_in_deg += 360;
  ...
  if (angle_in_deg < 0 || angle_in_deg >= 360) {
    Debug("CosLookupDegree error:" + std::to_string(angle_in_deg));
  }
  return cos_lookup_table_[angle_in_deg * kSinCosTableResolution];
}

float Driver::SinLookupDegree(float angle_in_deg) {
  if (angle_in_deg < 0) {
    angle_in_deg += 360;
  } else if (angle_in_deg >= 360) {
    angle_in_deg -= 360;
  ...
  if (angle_in_deg < 0 || angle_in_deg >= 360) {
    Debug("SinLookupDegree error:" + std::to_string(angle_in_deg));
  }
  return sin_lookup_table_[angle_in_deg * kSinCosTableResolution];
}

bool Driver::setIntrinsics(const std::vector<double>& elevation,
...
bool Driver::getIntrinsics(std::vector<double>& elevation,
...
bool Driver::parsePacket5_2(const uint8_t* data, size_t len,
                            std::shared_ptr<PointCloud> point_cloud) {
  auto udp_pkt = reinterpret_cast<const LidarPacket5_2*>(data);
  // check len
  if (udp_pkt->header.data_size + (28 + 8) != len) {
    Debug(
        "Invalid packet, packet size need be data size + 28 + 8, packet size:" +
        std::to_string(len) +
  ...
  for (size_t i = 0; i < unit_column_num; i++) {
    ...
    if (data_offset > data_size) {
      Debug("Invalid packet, column unit data overflow, start unit id: " +
            std::to_string(udp_pkt->unit_start_id) + ", unit column number: " +
...
bool Driver::ParsePacket5_1(const uint8_t* data, size_t len,
                            std::shared_ptr<PointCloud> point_cloud) {
  // check valid
