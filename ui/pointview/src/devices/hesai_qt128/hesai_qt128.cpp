#include "hesai_qt128.h"

#include "driver/driver_qt128.h"

HesaiQt128::HesaiQt128(
  ...
  // lidar intrinsics
  lidar_intrinsics_ =
      std::make_shared<autox::pointview::LidarIntrinsics>(device_context_, 128);
  ...
  // range image
  range_image_ = std::make_shared<autox::pointview::RangeImage>(device_context_,
                                                                3600, 128);
  ...
  return_settings.push_back({0x37, {"strongest"}});
  return_settings.push_back({0x38, {"last"}});
  return_settings.push_back({0x39, {"last", "strongest"}});
  return_settings.push_back({0x3B, {"last", "first"}});
  return_settings.push_back({0x3C, {"first", "strongest"}});
...
void HesaiQt128::initDriver() {
  ...
  // driver
  driver_ = std::make_shared<autox::drivers::hesai::DriverQt128>();
...
void HesaiQt128::convertToPclPointCloud(Frame& frame) {
...
bool HesaiQt128::getPointInfo(size_t idx, std::vector<double>& data) {
...
bool HesaiQt128::getPointUID(size_t idx, int& uid) {
  if (idx >= current_frame_.raw_pointcloud->points.size()) {
    return false;
  }
  auto& p = current_frame_.raw_pointcloud->points[idx];
...
bool HesaiQt128::initFromConfig(std::shared_ptr<autox::pointview::Config>
...
void HesaiQt128::initPointFilterSetting(
    autox::pointview::PointFilterSetting& point_filter_setting) {
...
bool HesaiQt128::updatePlayerState(autox::pointview::PlayerCmd cmd) {
