...
RecordReader::RecordReader(const std::string& file) {
  ...
  if (!file_reader_->Open(file)) {
    std::cout << "Failed to Open record file: " << file << std::endl;
    return;
  }
  ...
  if (file_reader_->ReadIndex()) {
    ...
    for (int i = 0; i < index_.indexes_size(); ++i) {
      ...
      if (!single_idx->has_channel_cache()) {
        std::cout << "Single channel index does not have channel_cache."
                  << std::endl;
        continue;
      }
...
proto::RecordDebugInfo RecordReader::GetDebugInfo() {
  proto::RecordDebugInfo record_debug_info;
  record_debug_info.mutable_index()->CopyFrom(index_);
  return record_debug_info;
}
...
bool RecordReader::ReadNextChunk(uint64_t begin_time, uint64_t end_time) {
  ...
  while (!reach_end_) {
    ...
    if (!file_reader_->ReadSection(&section)) {
      std::cout << "Failed to read section, file: " << file_reader_->GetPath();
      return false;
    }
    switch (section.type) {
      ...
      case SectionType::SECTION_CHANNEL: {
        // std::cout << "Read channel section of size: " << section.size;
        Channel channel;
        if (!file_reader_->ReadSection<Channel>(section.size, &channel)) {
          std::cout << "Failed to read channel section." << std::endl;
          return false;
        }
        break;
      }
      case SectionType::SECTION_CHUNK_HEADER: {
        // std::cout << "Read chunk header section of size: " << section.size;
        ChunkHeader header;
        if (!file_reader_->ReadSection<ChunkHeader>(section.size, &header)) {
          std::cout << "Failed to read chunk header section." << std::endl;
          return false;
        }
      ...
      case SectionType::SECTION_CHUNK_BODY: {
        ...
        if (!file_reader_->ReadSection<ChunkBody>(section.size, chunk_.get())) {
          std::cout << "Failed to read chunk body section." << std::endl;
          return false;
        }
        return true;
      }
      default: {
        std::cout << "Invalid section, type: " << section.type
