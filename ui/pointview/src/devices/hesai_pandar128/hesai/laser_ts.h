...
// this class is to correct azimuth and elevation angles cause by offset from
// lidar frame(rotation axis) and optical center
// see https://autoxai.atlassian.net/browse/INT-145
class LasersTSOffset {
 public:
  explicit LasersTSOffset(const std::string &sFile);
  ~LasersTSOffset();

  void setFilePath(std::string sFile);
  float getTSOffset(int nLaser, int nMode, int nState, float fDistance,
                    int nMajorVersion = 1);
  int getBlockTS(int nBlock, int nRetMode, int nMode, int nLaserNum);
  float getAngleOffset(float nTSOffset, int speed, int nMajorVersion = 1);
  float getAzimuthOffset(float azimuth_offset, float distance);
  float getPitchOffset(float pitch, float distance);
 ...
 private:
  ...
  void fillVector(char *pContent, int nLen, std::vector<int> &vec); // NOLINT
