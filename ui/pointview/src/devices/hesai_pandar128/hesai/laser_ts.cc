...
LasersTSOffset::LasersTSOffset(const std::string& sFile) {
  mBInitFlag = false;
  mNLaserNum = 0;
  ...
  for (int j = 0; j < CIRCLE; j++) {
    ...
    mSinAllAngleHB[j] =
        sinf(A_TO_R * angle) * sqrtf(PANDAR128_COORDINATE_CORRECTION_B *
                                         PANDAR128_COORDINATE_CORRECTION_B +
                                     PANDAR128_COORDINATE_CORRECTION_H *
                                         PANDAR128_COORDINATE_CORRECTION_H);
    mSinAllAngleH[j] = sinf(A_TO_R * angle) * PANDAR128_COORDINATE_CORRECTION_H;
  }

  for (int j = 0; j < PAI_ANGLE; j++) {
    mArcSin[j] = asinf(static_cast<float>(j - HALF_PAI_ANGLE) / HALF_PAI_ANGLE);
  }

  mShortOffsetIndex.resize(100);
  mLongOffsetIndex.resize(100);
  m_fArctanHB = atanf(PANDAR128_COORDINATE_CORRECTION_B /
                      PANDAR128_COORDINATE_CORRECTION_H) + 0.5f;
...
void LasersTSOffset::setFilePath(std::string file) {
  ...
  if (firstLine.size() == 2) {
    ...
    while (std::getline(fin, line)) {
      ...
      // printf("chnIdCDB, firetimeCDB:[%d][%f]  ",
      //        chnIdCDB, firetimeCDB);
...
void LasersTSOffset::fillVector(char *pContent, int nLen,
                                std::vector<int> &vec) {
  char *pNext = strtok(pContent, ","); // NOLINT
  ...
  while (pNext != NULL) {
    ...
    pNext = strtok(NULL, ","); // NOLINT
  }
}

float LasersTSOffset::getTSOffset(int nLaser, int nMode, int nState,
                                  float fDistance, int nMajorVersion) {
...
int LasersTSOffset::getBlockTS(int nBlock, int nRetMode,
                               int nMode, int nLaserNum) {
  switch (nLaserNum) {
...
float LasersTSOffset::getAngleOffset(float nTSOffset, int speed,
                                     int nMajorVersion) {
  switch (nMajorVersion) {
    case 1:
...
float LasersTSOffset::getAzimuthOffset(float azimuth_offset, float distance) {
  int  angle = static_cast<int>(100 * (m_fArctanHB + azimuth_offset));
  ...
  if (value < -1 || value > 1)
    return 0;
  int index = static_cast<int>(value * HALF_PAI_ANGLE) + HALF_PAI_ANGLE;
  if (index < 0 || index > PAI_ANGLE -1)
    return 0;
  return -mArcSin[index];
}

float LasersTSOffset::getPitchOffset(float pitch, float distance) {
