...
PcapUdpRecorder::PcapUdpRecorder(
...
void PcapUdpRecorder::stopRecordTask() {
  ...
  if (save_thread_) {
    ...
    QProgressDialog progress("saving record file, in progress...", "Abort", 0,
                             100, nullptr);
...
void PcapUdpRecorder::startRecordTask() {
  save_thread_ = std::make_unique<std::thread>([this]() {
    ...
    if (file.empty()) {
      LOG(INFO) << "error!!!! record file path is not set!!!!!";
      return;
    }
    time_t rawtime;
    struct tm* ptminfo;
    ...
    printf("current: %02d-%02d-%02d %02d:%02d:%02d\n", ptminfo->tm_year + 1900,
           ptminfo->tm_mon + 1, ptminfo->tm_mday, ptminfo->tm_hour,
    ...
    while (is_recording_ || !recorder_queue_done.empty()) {
      ...
      if (!is_recording_) {
        double rate_temp =
            1 - (double)recorder_queue_done.size() / (double)size_left_total;
        device_context_->updateRecordRate(rate_temp);
      }
      std::vector<uint8_t> data_v;
      if (recorder_queue_done.get(data_v)) {
        LOG(INFO) << "warnning!! recorder_queue_done is empty, get timeout";
        break;
      }
      ...
      memcpy(buffer + PCAP_UDP_PACKET_HAEADER_LENGTH, data_v.data(), data_size);
      recorder_queue_free.put(data_v);
  ...
  if (is_recording_) {
    ...
    if (wait_record_) {
      if (recorder_queue_free.size() > 20000 * 10 * 10) {
        LOG(INFO) << "==========start Record Task again=======";
        wait_record_ = false;
